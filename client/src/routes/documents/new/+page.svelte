...
<div class="w-full max-w-md mx-auto p-6">
	<h2 class="text-3xl font-bold m-10">Загрузка файла</h2>
	<form on:submit|preventDefault={handleSubmit}>
		<div class="w-42">
			<label for="file-input" class="sr-only">Выберите файл</label>
			<input
				bind:files
				type="file"
				name="file-input"
		...
		{#if loading && !$documents.error}
			<Progress progress={$documents.uploadProgress}>
				<Alert type="success">Загрузка завершена! Возвращаемся к списку...</Alert>
			</Progress>
		{/if}

		{#if $documents.error}
			<Alert>Ошибка: {$documents.error}</Alert>
		{/if}

		{#if !loading}
			<Button className="w-full my-3" disabled={loading}>Отправить</Button>
		{/if}
	</form>
</div>
